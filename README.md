...
### Project Overview
This project uses Apache Airflow for orchestration along with Spark (AWS EMR) to process data which is then stored in an AWS Redshift database. The project uses data generated by the GDELT Project to determine the stability of different regions around the world. 
...
In order to support the end goal of querying, analyzing, and reporting aggregated statistics, a dimensional model consisting of fact and dimension tables has been implimented.  The stats and ratings gathered can be used
...
can determine how stable a region/country is and use that information as part of the decision to expand to a region/country.
...
key feature is its ability to easily build scheduled data pipelines using a flexible Python framework.
...
a period of time and/or at a particular location.  Or what are the different types of events that occured in one region.  The fact and 
...
#### Dataset
The size of the dataset is approx 5GB. The dataset holds data from 2 months (Jan, Feb - 2019).  
...
#### Execution
Once setup is complete, launch Airflow to run the dags.  Currently there are 3 dags (but the plan is to
